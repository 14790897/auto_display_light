...
> **两种硬件方案对比**
> - **方案 A（主仓库默认）**：ESP32-C3 开发板 + TEMT6000 模块 + 杜邦线连接 → 固件用 `esp32c3.yaml`（Arduino，电压正比）（接线见[主 README 硬件接线](../README.md#硬件接线)）
> - **方案 B（本目录）**：集成版 PCB，TEMT6000 直接焊在 ESP32-C3 最小系统板上，更小体积、更便携 → 固件用 `temt-on-esp32c3.yaml`（ESP-IDF，共射极电压反比）
...
| **光传感器** | GPIO3 连接 TEMT6000（共射极电路，光越强电压越低），读取电压数据判断光照强度 |

### 固件配置（重要）

集成版 PCB **必须使用 `temt-on-esp32c3.yaml`**，不要用 `esp32c3.yaml`：

| 配置文件 | 电路 | 转换公式 | 是否适用集成版 |
|----------|------|----------|----------------|
| `esp32c3.yaml` (Arduino) | 模块带放大，电压正比 | `pct = V/3.3*100` | ❌ 会读反 |
| `temt-on-esp32c3.yaml` (ESP-IDF) | 共射极，电压反比 | `pct = (1-V/3.3)*100` | ✅ 正确 |

集成版固件额外包含 GPIO12/13 板载 LED 控制、WiFi 发射功率限制（C3 功率过高会死机）、USB-Serial/JTAG 日志输出。设备名为 `temt6000-integrated`。

> CI 已自动构建集成版固件，Release 页面下载 `firmware-integrated.factory.bin` 即可。
...
2. **焊接元器件**：参考上方焊接工艺章节
3. **烧录固件**：使用 `temt-on-esp32c3.yaml` 编译固件并烧录（⚠️ 不要用 `esp32c3.yaml`，两者光照转换公式相反）。可直接从 [Release](../releases) 下载 `firmware-integrated.factory.bin`，烧录方法见[主 README](../README.md#方法-1esphome-自动上传最简单)
4. **配置联网**：ESPHome 配置 WiFi，板子直接联网
5. **查看数据**：浏览器打开 `http://temt6000-integrated.local` 即可看到光敏传感器实时数据
